...
HOOK_DEF(void*, do_dlopen_V19, const char *filename, int flag, const void *extinfo) {
    ...
    onSoLoaded(filename, ret);
    ALOGD("do_dlopen --19: %s, return : %p.", redirect_path, ret);
    FREE(redirect_path, filename);
    return ret;
}
...
HOOK_DEF(void*, do_dlopen_V24, const char *name, int flags, const void *extinfo, void *caller_addr) {
    ...
    onSoLoaded(name, ret);
    ALOGD("do_dlopen--24 : %s, return : %p.", redirect_path, ret);
    FREE(redirect_path, name);
    return ret;
}
//HOOK_DEF(void*,memcpy,void *buf1, const void *buf2, unsigned int count){
//    void *ret= (void *) orig_memcpy(buf1, buf2, count);
//    char *buffer=(char *)buf2;
//    FILE *file=fopen("/sdcard/hookdex/123.dex", "wb");
//    ALOGD("memcpy : %p",buffer );
